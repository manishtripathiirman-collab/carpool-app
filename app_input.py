...
with tab_trip:
    ...
    # Clear validation state immediately when the date changes
    if st.window_events if 'last_travel_date' not in st.session_state or st.session_state.last_travel_date != str(travel_date):
    ...
    # Form rows stay completely active and never vanish!
    ...
    # Persistent Warning Placements (Rendered right above action triggers)
...
with tab_expense:
    ...
    if st.session_state.just_saved_exp:
        ...
        # Expense Warnings Layout placement
    ...
    if st.session_state.is_admin:
        ...
        if not df_exp_existing.empty:
            ...
            if st.button("🔥 PURGE EXPENSE ENTRY", key="purge_exp_btn"):
                ...
                payload_exp_del = {"message": "Admin Purged Expense row", "content
