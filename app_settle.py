...
# 1. Process Commute Sheets
if not df_trips.empty:
    for _, row in df_trips.iterrows():
        driver = str(row.get("Driver", "")).replace('"', '').replace("'", "").strip().title()
        full_list = [p.strip().title() for p in str(row.get("Full Day Passengers", "")).replace('"', '').replace("'", "").split(",") if p.strip() and p.strip().lower() != 'none']
        half_list = [p.strip().title() for p in str(row.get("Half Day Passengers", "")).replace('"', '').replace("'", "").split(",") if p.strip() and p.strip().lower() != 'none']
...
if not df_expenses.empty:
    for _, row in df_expenses.iterrows():
        ...
        consumer_list = [p.strip().title() for p in str(row.get("Shared By", "")).replace('"', '').replace("'", "").split(",") if p.strip()]
...
with col_left:
    ...
    if not df_trips.empty: st.dataframe(df_trips, use_container_width=True)
    else: st.info("No recorded travel logs found.")
with col_right:
    st.markdown("#### 💰 Shared Expenses History Logs")
    if not df_expenses.empty: st.dataframe(df_expenses, use_container_width=True)
    else: st.info("No recorded expense logs found.")
