...
df_trips, df_expenses = df_trips_raw.copy(), df_expenses_raw.copy()
    ...
    for p2 in all_commuters:
        if p1 != p2 and (p1, p2) not in processed_pairs and (p2, p1) not in processed_pairs:
            ...
            if p1_owes_p2 > p2_owes_p1:
                ...
                if net_debt > 0.01: final_settlements.append((p2, p1, net_debt))
...
st.markdown(f'<div class="scorecard-row"><div class="scorecard-box"><div class="scorecard-label">👑 King of Wheel</div><div class="scorecard-val">{top_driver} ({driver_counts.get(top_driver, 0)} Days)</div></div><div class="scorecard-box"><div class="scorecard-label">🎒 Top Passenger</div><div class="scorecard-val">{top_passenger} ({passenger_counts.get(top_passenger, 0)} Rides)</div></div></div>', unsafe_allow_html=True)
...
with tab_payout:
    ...
    if not final_settlements:
        ...
        for deb, cred, amt in final_settlements:
            st.markdown(f'<div class="pairwise-card"><div class="pairwise-label-set"><div class="payer-info">👉 {deb}</div><div class="payer-sub">Pays directly to <b>{cred}</b></div></div><div class="payout-pill">₹{amt:,.0f}</div></div>', unsafe_allow_html=True)
