...
if not df_trips.empty:
    ...
    # Isolate independent data calculation layers
    ...
    # Calculate Carpool Settlements Pairs
    cp_settlements = []
    ...
    # Calculate Expense Settlements Pairs
    misc_settlements = []
    for i in range(len(commuters)):
        for j in range(i + 1, len(commuters)):
            p1, p2 = commuters[i], commuters[j]
    ...
    with tab_summary:
        ...
        # WhatsApp Link Text Generator Block
    ...
    # TAB 2: DETAILED RECORD EXPLORER SPREADSHEET
    with tab_ledger:
        st.markdown(f"### 📋 Full Bill Ledger Breakdown (Selected Total: ₹{total_period_expenses:,.2f})")
        if not filtered_expenses.empty:
        ...
        st.markdown("---")
        with st.expander("📱 View Raw Travel History Calendar Logs"):
