...
st.markdown(
    ...
    .block-container {
        ...
        border: 1px solid rgba(255, 255, 255, 0.1) !important; margin-top: 20px !important;
    }
...
st.markdown('<p class="main-title">📊 MG Carpool Settlement Engine (Diagnostic Mode)</p>', unsafe_allow_html=True)
...
# LIVE TRACKING DIAGNOSTIC HEADERS
st.markdown("### 🔍 Live Data Stream Diagnostics")
...
if TOKEN and REPO:
    try:
        ...
        with col1:
            st.warning(f"Travel File API Server Response Status: Code {r.status_code}")
        if r.status_code == 200:
            df_trips = pd.read_csv(io.StringIO(base64.b64decode(r.json()["content"]).decode("utf-8")))
            with col1:
                st.success(f"Successfully downloaded travel data! Total rows discovered: {len(df_trips)}")
        
        r_e = requests.get(f"{EXPENSE_URL}?cb={random.randint(1, 1000000)}", headers=HEADERS)
        with col2:
            st.warning(f"Expense File API Server Response Status: Code {r_e.status_code}")
        if r_e.status_code == 200:
            df_expenses = pd.read_csv(io.StringIO(base64.b64decode(r_e.json()["content"]).decode("utf-8")))
            with col2:
                st.success(f"Successfully downloaded expense data! Total rows discovered: {len(df_expenses)}")
    except Exception as e:
        st.error(f"Critical execution block crash: {e}")
else:
    st.error("Secrets missing! Check GITHUB_TOKEN and GITHUB_REPO parameters inside Streamlit dashboard panels.")
...
if not df_trips.empty:
    for _, row in df_trips.iterrows():
        ...
        trip_total = 0.0
        for p in full_list:
            if p in balances: balances[p] -= 300.0; trip_total += 300.0
        for p in half_list:
            if p in balances: balances[p] -= 150.0; trip_total += 150.0
        if driver in balances: balances[driver] += trip_total
...
if not df_expenses.empty:
    for _, row in df_expenses.iterrows():
        ...
        try: total_amount = float(total_amount)
...
for idx, name in enumerate(all_commuters):
    ...
    with cols[idx]:
        if val > 0: v_class, p_sign = "val-positive", "+"
        elif val < 0: v_class, p_sign = "val-negative", ""
        else: v_class, p_sign = "val-zero", ""
