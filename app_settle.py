...
# Fallback Path Strategy: Checks root folder first, then searches folder trees
paths_to_test_logs = [
    f"https://api.github.com/repos/{REPO}/contents/carpool_logs.csv",
    f"https://api.github.com/repos/{REPO}/contents/data/carpool_logs.csv",
    f"https://api.github.com/repos/{REPO}/contents/src/carpool_logs.csv"
]

paths_to_test_exps = [
    f"https://api.github.com/repos/{REPO}/contents/carpool_expenses.csv",
    f"https://api.github.com/repos/{REPO}/contents/data/carpool_expenses.csv",
    f"https://api.github.com/repos/{REPO}/contents/src/carpool_expenses.csv"
]

if TOKEN and REPO:
    # Fetch Travel Records
    for url in paths_to_test_logs:
        try:
            r = requests.get(f"{url}?cb={random.randint(1, 1000000)}", headers=HEADERS)
    ...
    for url in paths_to_test_exps:
        try:
            r_e = requests.get(f"{url}?cb={random.randint(1, 1000000)}", headers=HEADERS)
